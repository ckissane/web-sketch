(function() {
  ...
  function drawNode(node, c, root) {
    ...
    if (style) {
      ...
      genCond.push(style.transform + "");
      genCond.push(style.visibility + "");
      genCond.push(style.display + "");
    }
    ...
    if (needToGen) {
      ...
      if (node.getBoundingClientRect) {
        ...
        if (style.backgroundColor !== "rgba(0, 0, 0, 0)") {
          gens.push(generator.rectangle(rect.x, rect.y, rect.width, rect.height, {
        ...
        if (style.borderTopWidth !== "0px") {
          gens.push(generator.line(rect.x, rect.y, rect.x + rect.width, rect.y, {
            stroke: style.borderTopColor || "rgba(0,0,0,0)",
            strokeWidth: style.borderTopWidth,
            bowing: bow / rect.width
          }));
        }
        if (style.borderLeftWidth !== "0px") {
          gens.push(generator.line(rect.x, rect.y, rect.x, rect.y + rect.height, {
            stroke: style.borderLeftColor || "rgba(0,0,0,0)",
            strokeWidth: style.borderLeftWidth,
            bowing: bow / rect.height
          }));
        }
        if (style.borderBottomWidth !== "0px") {
          gens.push(generator.line(rect.x, rect.y + rect.height, rect.x + rect.width, rect.y + rect.height, {
            stroke: style.borderBottomColor || "rgba(0,0,0,0)",
            strokeWidth: style.borderBottomWidth,
            bowing: bow / rect.width
          }));
        }
        if (style.borderRightWidth !== "0px") {
          gens.push(generator.line(rect.x + rect.width, rect.y, rect.x + rect.width, rect.y + rect.height, {
            stroke: style.borderRightColor || "rgba(0,0,0,0)",
            strokeWidth: style.borderRightWidth,
            bowing: bow / rect.height
          }));
        }
      }
    ...
    if (node.childNodes && node.getBoundingClientRect) {
      var child = [];
